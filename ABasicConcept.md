...
# Function Call（函数调用 / 工具调用）：
     ...
     让大模型不再只做文本生成，能够理解用户意图，主动选择调用外部程序 / 函数 / API，拿到外部结果之后，再整理成自然语言回答用户。


# Agent 短期记忆和长期记忆

     短期记忆= 当前任务的工作内存 长期记忆= 所有历史的经验库
...
```text
     通过调用模型的正确率、任务完成率，调用链条的合理性来作为奖励函数，引导模型来进行更高质量的交互
```
